...
print()
print()
print('DETECTION BY ALA SCANNING TEST 1:')
print('----------------------------------------------------------------')
            ...
            for atom2 in st[0]['E'][Interface_ResiduesE[j]].get_atoms():
                ...
                if atom1.serial_number < atom2.serial_number: # to check that the atom serial number is always smaller in the first atom than the second 
                    ...
                    if distances <= interface_distan: # compare it with the default value that it is 8 Angstroms
                        ...
                        Val1+= Eelec(distances, ch1, ch2)
                        Val2 += Evdw(distances, sig1, sig2, eps1, eps2)
                        Val3 += Esolvation(sigma,Asa)
                        Val4 += Esolvation(sigma2,Asa2)
                        Val5 += Total_Solvation


        NORMAL.append( [st[0]['A'][Interface_ResiduesA[i]].get_resname(),st[0]['E'][Interface_ResiduesE[j]].get_resname(),Val1+ Val2 - Val3 -Val4 +Val5] )
        Val11=0
        Val22=0
        Val33=0
        Val44=0
        Val55=0
        for res1 in st[0]['A'][Interface_ResiduesA[i]].get_resname():
            if res1 != 'ALA':
                
                for res2 in st[0]['E'][Interface_ResiduesE[j]].get_resname():
                    if res2 != 'ALA':
                            ...
                            for atom2 in st[0]['E'][Interface_ResiduesE[j]].get_atoms():
                                ...
                                if atom1.serial_number < atom2.serial_number: # to check that the atom serial number is always smaller in the first atom than the second 
                                    ...
                                    if distances <= interface_distan: # compare it with the default value that it is 8 Angstroms
                                        ...
                                        Val11 += Eelec(distances, ch1, ch2)
                                        Val22 += Evdw(distances, sig1, sig2, eps1, eps2)
                                        Val33 += Esolvation(sigma,Asa)
                                        Val44 += Esolvation(sigma2,Asa2)
                                        Val55 += Total_Solvation

        WILD.append(  [ st[0]['A'][Interface_ResiduesA[i]].get_resname(), st[0]['E'][Interface_ResiduesE[j]].get_resname() ,Val11+ Val22 - Val33 -Val44 +Val55])
...
for x in NORMAL:
    for y in WILD:
        if x[3] > y[3]:
            print("Normal is better than the Wild type")
        elif x[3] == y[3]:
            print("There isnt any change inside this pairs")
        else:
            print("Wild is better than the Normal type")
